...
# FYI from google : Out-of-bag error, also called out-of-bag estimate, is a method of measuring the prediction error of random forests, boosted decision trees, and other machine learning models utilizing bootstrap aggregating.
# Class Reliance (MCR)

# Random Forest model acccuracy,   OOB estimate of  error rate: 11.83%, which means Accuracy
#Here the Out of Bag error (OOB) gives us the miscalssification rate (MCR) of the model. In this case it comes out to #be 11.83%, which gives us the accuracy of 88.17%
rf_adult_model<-randomForest(income~.,data = train.data)
rf_adult_model
# Plot To check classwise error
...
#LDA with all predictors, Accuracy 85.57%
ldafit <- lda(income~., data=train.data)
ldapred <- predict(ldafit, newdata=test.data)
ldatable <- table(ldapred$class, test.data$income)
lda.acc <- mean(ldapred$class==test.data$income)

# LDA excluding race and sex  acuuracy little lower: 85.50
ldafit <- lda(income~.-race-sex, data=train.data)
ldapred <- predict(ldafit, newdata=test.data)
ldatable <- table(ldapred$class, test.data$income)
lda.acc2 <- mean(ldapred$class==test.data$income)
